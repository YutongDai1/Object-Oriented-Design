...
public class Game {
  ...
  public Game(Grid grid, int connectN, int targetScore) {
    ...
    this.players = new Player[] {
        new Player("Player 1", GridPositionStatus.YELLOW),
        new Player("Player 2", GridPositionStatus.RED)
    };
  ...
  private void printBoard() {
    ...
    for (int i = 0; i < grid.length; i++) {
      ...
      for (int piece : grid[i]) {
        if (piece == GridPositionStatus.EMPTY.ordinal()) {
          row.append("0 ");
        } else if (piece == GridPositionStatus.YELLOW.ordinal()) {
          row.append("Y ");
        } else if (piece == GridPositionStatus.RED.ordinal()) {
          row.append("R ");
        }
      }
  ...
  private Player playRound() {
    while (true) {
      for (Player player : this.players) {
        ...
        int row = pos[0];
        int col = pos[1];
        GridPositionStatus pieceColor = player.getPieceColor();

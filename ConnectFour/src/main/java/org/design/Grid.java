...
public class Grid {
  ...
  public void initGrid() {
    ...
    for (int i = 0; i < rows; i++) {
      for (int j = 0; j < columns; j++) {
        grid[i][j] = GridPositionStatus.EMPTY.ordinal();
      }
    }
  }
  ...
  public int placePiece(int column, GridPositionStatus piece) {
    if (column < 0 || column >= this.columns) {
      throw new Error("Invalid column");
    }
    if (piece == GridPositionStatus.EMPTY) {
      throw new Error("Invalid piece");
    }
    // Place piece in the lowest empty row
    for (int row = this.rows - 1; row >= 0; row--) {
      if (this.grid[row][column] == GridPositionStatus.EMPTY.ordinal()) {
  ...
  public boolean checkWin(int connectN, int row, int col, GridPositionStatus piece) {
    // Check horizontal
    int count = 0;
